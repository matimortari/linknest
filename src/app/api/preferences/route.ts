...
export async function GET() {
	...
	try {
		...
		return NextResponse.json({ error: "Failed to retrieve user preferences", details: String(err) }, { status: 500 })
	}
}
...
export async function PUT(req: NextRequest) {
	...
	try {
		...
		return NextResponse.json({ error: "Failed to update preferences", details: String(err) }, { status: 500 })
	}
}
