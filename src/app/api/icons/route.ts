...
export async function GET() {
	...
	try {
		...
		return NextResponse.json({ error: "Failed to retrieve icons", details: String(err) }, { status: 500 })
	}
}
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		return NextResponse.json({ error: "Failed to create icon", details: String(err) }, { status: 500 })
	}
}
...
export async function DELETE(req: NextRequest) {
	...
	if (!id) {
		return NextResponse.json({ error: "Missing or invalid icon ID" }, { status: 400 })
	}

	try {
		const existingIcon = await db.userIcon.findUnique({ where: { id } })
		if (!existingIcon || existingIcon.userId !== session?.user.id) {
			return NextResponse.json({ error: "Social icon not found or unauthorized" }, { status: 404 })
		}

		await db.userIcon.delete({ where: { id } })

		return NextResponse.json({ message: "Icon deleted successfully", id }, { status: 200 })
	} catch (err) {
		return NextResponse.json({ error: "Failed to delete icon", details: String(err) }, { status: 500 })
	}
}
