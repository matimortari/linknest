<template>
  <div class="flex w-full flex-row items-center justify-between gap-4 rounded-2xl border p-2 text-sm">
    ...
    <select
      :id="id" :value="value" :disabled="disabled" class="text-label scale-sm cursor-pointer bg-transparent p-2 text-muted-foreground outline-none disabled:opacity-50"
      @change="$emit('update:value', $event.target && ($event.target as HTMLSelectElement).value)"
    >
      <option v-for="option in options" :key="option.value" :value="option.value" class="bg-card text-center">
        {{ option.label }}
      </option>
    </select>
  </div>
