export function useActiveHeading(options: { selector?: string, parseMethod?: boolean } = {}) {
  ...
  onMounted(async () => {
    ...
    headers.value = Array.from(hElements).map((el) => {
      ...
      // Check the next sibling element for method
      if (parseMethod) {
  ...
  function headerClasses(header: any) {
    const classes = []
    if (header.level === 2)
      classes.push("ml-0 text-base")
    if (header.level === 3)
      classes.push("ml-4 text-sm")
    if (header.level === 4)
